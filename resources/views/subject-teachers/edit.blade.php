...
@section('header', 'Edit Guru Mapel')
            ...
            <div class="mb-3">
                ...
                <select name="guru_id" class="form-control @error('guru_id') is-invalid @enderror" required>
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ $subjectTeacher->guru_id == $teacher->id ? 'selected' : '' }}>{{ $teacher->nama }} ({{ $teacher->nip }})</option>
                    @endforeach
                </select>
                @error('guru_id')
            ...
            <div class="mb-3">
                ...
                <select name="mapel_id" class="form-control @error('mapel_id') is-invalid @enderror" required>
                    ...
                    @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ $subjectTeacher->mapel_id == $subject->id ? 'selected' : '' }}>{{ $subject->mapel }}</option>
                    @endforeach
                </select>
                @error('mapel_id')
                    <span class="invalid-feedback">{{ $message }}</span>
